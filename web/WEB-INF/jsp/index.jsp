...
<head>
	...
	<%FormHelper fh = new FormHelper();
			...
			HashMap reassignUserNames = fh.getReassignUsers();
...
<li>			
	...
	<table width="90%" align="center">
		<tr>
			<td align="center">
				<a href="mailto:ukapps.support@sonymusic.com?bcc=owen.giles@sonymusic.com.com&subject=Project Memo Suggestions and Feedback&title="Outlook must be open to use this function.">PROVIDE FEEDBACK/ GET HELP </a>
			</td>
		</tr>
	</table>	
	<table>	
		...
		<legend style="font-weight: bold;font-size: 16;COLOR:black; text-decoration: underline;">		
				...
				<div id="openPMs">
					<table width="610px" border="0" style="table-layout: fixed;">
					...
					<div style="height: 650px; width: 100%; overflow: auto ;overflow-x:hidden">
						<table width="600px" border="0" style="table-layout: fixed; align:left">
							<%HashMap editingParams = null;
			HashMap deletingParams = null;
			HashMap reassignDrafts = null;
			...
			<% }%>
				...
				%>
								...
								<td class="ic">
								 ...
								 <%-->if user is one of the production team --%>
								 <% ArrayList assignableUsers = fh.getReassignableUsersList(); 							
								 	if (pmUser.getId()!=null && (assignableUsers.contains(pmUser.getId()))){
										if(lockedMemo.getRevisionID().equals("1")){%>		
									<html:form method="post" name="draftForm" type="com.sonybmg.struts.pmemo3.form.DraftForm" action="/reassignDraft.do">
												<html:select property="draftAssignee" title="draftAssignee" styleId="draftAssignee">	
												<html:option value=""></html:option>								
													<%Iterator draftsIter = reassignUserNames.keySet().iterator();
														while (draftsIter.hasNext()) {			
															String userID = (String) draftsIter.next();				
															String userName = (String) reassignUserNames.get(userID);%>
													<html:option value="<%=userID%>"><%=userName%></html:option>
														<%}%>
												</html:select>	
									<html:hidden property="memoRef" value="<%=lockedMemo.getMemoRef()%>" />
									<html:image page="/images/insert.gif" border="0" alt="submit" />
							    </html:form> 
								 <%}} %>
								</td>
							</tr>

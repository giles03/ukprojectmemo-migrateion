					...
					if((format.equals("Video")) || (format.equals("Pseudo Video")) || (format.equals("Download"))){						
					   if(pm2.getTrackName()!=null){						   
					    	  productLink = productLink+pm2.getTrackName()+" ";
					    	 	 if(pm2.getTrackComments()!=null) {					    			
				    				if(pm2.getTrackComments().toLowerCase().contains("clean")){
							   		productLink = productLink+" (Clean) ";
							   	}
					    	}	  
						}
...
<td width="25%">
	...
	<td colspan="2"><%=pm2.getSupplementTitle()%></td></tr>	
	<%}
...
<% if ((fh.isMemoCurrentlyBeingEdited(pm2.getMemoRef()).equals("Y") && (!fh.isCurrentUserEditingDraft(pm2.getMemoRef(),userName)))
	|| (userRole.equals(Consts.VIEW)) || fh.isMemoAutogenerated(pm2.getMemoRef())) {%>
						...
						<fieldset style="border-top-style: none; border: thin solid #cccccc; width: 90%">
							...
							<table style="width: 100%" align="center">
								<tr>
									...
									<td width="35%" align="right"><span style="font-size: 14; padding-right: 15px"><u><b>Start Date</b></u></span></td>
		...
		if (preOrder.getPreOrderDate() != null && (!preOrder.getPreOrderDate().equals(""))) {
				...
				modifiedPreOrderDate = dateFormat.format(preOrderDate);	  
								...
								<%} %>	
									...
									<td align="right"><span style="font-size: 14; padding-right: 15px"><%=modifiedPreOrderDate%></span></td>
								</tr>
		<%}%>						
	  						</table>
						</fieldset>
	...
	<td>	
		...
		if (pm2.getPullDate() != null ) {
			modifiedPullDate = "";
			Date pullDate = java.sql.Date.valueOf(pm2.getPullDate().substring(0, 10));
			modifiedPullDate = dateFormat.format(pullDate);
		%> 	
			<tr>
				<td colspan="2">
					<fieldset style="border-top-style: none; border: thin solid #ff3300; width: 90%">
						<legend style="text-decoration: underline; font-weight: bold; font-size: 14">PULL DATE</legend>
						<table style="width: 100%" align="center">
							<tr>
								<td width="65%"><span style="font-size: 14"><u><b>Partner</b></u></span></td>
								<td width="35%" align="center"><span style="font-size: 14;"><u><b>Pull Date</b></u></span></td>
							</tr>							
							<tr>
								<td><span style="font-size: 14"><%=pm2.getPullPartner()%></span></td>
								<td><span style="font-size: 14"><%=modifiedPullDate%></span></td>									
							</tr>
  						</table>
					</fieldset>
				</td>

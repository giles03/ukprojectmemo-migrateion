   ...
   if (trackList != null) {
		...
		<html:form name="myForm" method="POST" action="/editTrack.do" type="com.sonybmg.struts.pmemo3.form.TracksForm" >
			<tr>	
		   			...
		   			<td>		   		
		   				<html:text property="trackName" size="72" maxlength="100" value="<%=track.getTrackName()%>" onchange="submit()"></html:text>		   				
		   			</td>
		   			<td>		   		
		   				<html:text property="isrcNumber" size="18" maxlength="20" value="<%=track.getIsrcNumber()%>" onchange="submit()" ></html:text>
		   			</td>
				    <%if (pm.getConfigurationId()!=null && pmDAO.isProductInMobile(pm.getConfigurationId())) {%>
		   			<td>		   		
		   				<html:text property="gridNumber" size="18" maxlength="20" value="<%=track.getGridNumber()%>" onchange="submit()" ></html:text>
		   			</td>		   			
		   			<td>		   		
		   				<html:text property="comments" size="48" maxlength="400" value="<%= track.getComments() %>" onchange="submit()"  ></html:text>		   			
		   			</td>	
		   			<%} else {%>
		   			<td>		   		
		   				<html:text property="comments" size="33" maxlength="400" value="<%= track.getComments() %>" onchange="submit()"  ></html:text>		   			
		   			</td>
		   			<%} if (pm.getConfigurationId()!=null){%>	
		   			<td>		   		
		   				<html:text property="dspComments" size="33" maxlength="400" value="<%= track.getDspComments() %>" onchange="submit()"  ></html:text>		   			
		   			</td>		   			
		   			<%} %>				
		   		</tr>		   		
		</html:form>
	 ...
	 %>
		...
		<html:form name="editPreOrderTracksForm" method="POST" action="/editPreOrderTrack.do"  type="com.sonybmg.struts.pmemo3.form.TracksForm" >
				...
				<tr>	
		   			...
		   			<td >		   		
		   				<html:text property="trackName" size="73" maxlength="100" value="<%=preOrderTrack.getTrackName()%>" onchange="submit();" ></html:text>		   				   			
	
		   			</td>
		   			<td>		   		
		   				<html:text property="isrcNumber" size="18" maxlength="20" value="<%=preOrderTrack.getIsrcNumber()%>" onchange="submit()" ></html:text>		   				
		   			</td>
		   			 <%if (pm.getConfigurationId()!=null && pmDAO.isProductInMobile(pm.getConfigurationId())) {%>
		   			<td>		   		
		   				<html:text property="gridNumber" size="18" maxlength="20" value="<%=preOrderTrack.getGridNumber()%>" onchange="submit()" ></html:text>
		   			</td>		   			
		   			<td>		   		
		   				<html:text property="comments" size="48" maxlength="400" value="<%= preOrderTrack.getComments() %>" onchange="submit()"  ></html:text>		   			
		   			</td>	
		   			<%} else {%>
		   			<td>		   		
		   				<html:text property="comments" size="33" maxlength="400" value="<%= preOrderTrack.getComments() %>" onchange="submit()"  ></html:text>		   			
		   			</td>
		   			<td>		   		
		   				<html:text property="dspComments" size="33" maxlength="400" value="<%= preOrderTrack.getDspComments() %>" onchange="submit()" ></html:text>		   			
		   			</td>

...
<%FormHelper fh = new FormHelper();
			...
			if(fh.isMemoAutogenerated(memoRef)){
	...
	<fieldset style="border-top-style: none; border:thin solid #cccccc;width:100%">
		...
		<tr>
			...
			<% }else if (isBeingEdited){
				...
				if (userRole.indexOf(Consts.VIEW) == -1 && (!isAutogenerated)) { %>
			...
			<a href="http://memo-uat.smeukapps.com/css/?searchId=<%=pm.getMemoRef() %>&id=<%=currentUserId%>">
					...
					if (iteratorDigiKeys.hasNext()) {
						...
						while (iteratorDigiVals.hasNext()) {
							while (iteratorDigiKeys.hasNext()) {
								...
								Date date = null;
								if(res3.getDigitalReleaseDate()!=null){
			...
			<td><%=format%> </td> 
				...
				<%  if (user.getRole().indexOf(Consts.VIEW) == -1) {
					...
					<td colspan="4" align=center><html:link action="/emailLink.do" name="viewLinkParams">View</html:link></td>	
						...
						<%} else if ((fh.isCurrentUserCreatingDraft(memoRef, user.getId()))) { %>
			 ...
			 <% }
				...
				<% 			}
					 ...
					 if (user.getRole().indexOf(Consts.VIEW) == -1 && (!isAutogenerated)) {		
	...
	if (iterator.hasNext()) {
		while (iterator.hasNext()) {
			...
			<%}%> 
					...
					<td style="font-size:12px">&nbsp;<%=(res.getPhysNumberDiscs() != null ? res.getPhysNumberDiscs() : "&nbsp;")%></td>			
